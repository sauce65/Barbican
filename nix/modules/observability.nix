...
let
  ...
  dockerComposeConfig = pkgs.writeText "docker-compose.yml" ''
    ...
    services:
      # Init containers use network_mode: host to inherit the host's network stack
      # This is required for SLIRP DNS resolution in QEMU VMs
      loki-init:
        image: alpine:3.19
  ...
  observabilityStartScript = pkgs.writeShellScript "observability-start" ''
    ...
    # Use --network host to inherit the host's network stack (required for SLIRP DNS in QEMU VMs)
