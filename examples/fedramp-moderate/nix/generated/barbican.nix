...

{
  ...
  # Derived from profile: FedRAMP Moderate
  barbican.securePostgres = {
    enable = true;
    listenAddress = "127.0.0.1";
    allowedClients = [  ];
    database = "hello_fedramp_moderate";
    username = "hello_fedramp_moderate";
    passwordFile = config.age.secrets.db-password.path;

    # Transport Security (SC-8)
    enableSSL = true;
    enableClientCert = false;  # mTLS: not required for FedRAMP Moderate
  ...
  barbican.vmFirewall = {
    ...
    # Egress Filtering: recommended for FedRAMP Moderate
  ...
  # Application Service
  systemd.services.hello_fedramp_moderate = {
    description = "hello-fedramp-moderate";
    wantedBy = [ "multi-user.target" ];
    after = [ "network.target" "postgresql.service" ];

    serviceConfig = config.barbican.systemdHardening.presets.networkService // {
      ExecStart = "${pkgs."hello-fedramp-moderate"}/bin/hello_fedramp_moderate";
      EnvironmentFile = config.age.secrets.hello_fedramp_moderate-env.path;
      ...
      # Paths
      ReadWritePaths = [ "/var/lib/hello_fedramp_moderate" ];
    };
  };
}
