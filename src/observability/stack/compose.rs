...
fn generate_compose(config: &ComposeConfig, fedramp: &ObservabilityComplianceConfig, app_name: &str) -> String {
    ...
    // TLS-related config - skip in development mode
    let certs_volume = if is_dev { "" } else { "\n      - ./certs:/certs:ro" };
    let prometheus_web_config_cmd = if is_dev { "" } else { "\n      - '--web.config.file=/etc/prometheus/web.yml'" };
    let prometheus_web_config_vol = if is_dev { "" } else { "\n      - ./prometheus/web.yml:/etc/prometheus/web.yml:ro" };
    let alertmanager_web_config_cmd = if is_dev { "" } else { "\n      - '--web.config.file=/etc/alertmanager/web.yml'" };
    let alertmanager_web_config_vol = if is_dev { "" } else { "\n      - ./alertmanager/web.yml:/etc/alertmanager/web.yml:ro" };

    // Grafana environment - use defaults in development mode
    let grafana_env = if is_dev {
        r#"environment:
  ...
  {network_name}:
    ...
    let header_comment = if is_dev {
        format!(
            r#"# Docker Compose - {app_name} Observability Stack
# Profile: Development (no TLS/auth required)
# Generated by barbican observability stack
#
# Usage:
#   docker-compose up -d
#
# This is a development stack with no security hardening.
# For production, use --profile fedramp-moderate or higher."#,
            app_name = app_name
        )
    } else {
        format!(
...
#
# Prerequisites:
#   1. Generate certificates using Vault PKI
...
services:
  ...
  loki:
    ...
    volumes:
      ...
      - {volume_prefix}_loki_data:/loki/data{certs_volume}{loki_tmpfs}
    ports:
      - "3100:3100"
    networks:
      - {network_name}
  ...
  prometheus:
    ...
    command:
      ...
      - '--storage.tsdb.retention.size={retention_size}GB'{prometheus_web_config_cmd}
      - '--web.enable-lifecycle'
      - '--web.enable-admin-api'
    volumes:
      - ./prometheus/prometheus.yml:/etc/prometheus/prometheus.yml:ro{prometheus_web_config_vol}
      - ./prometheus/rules:/etc/prometheus/rules:ro
      - {volume_prefix}_prometheus_data:/prometheus{certs_volume}{prometheus_ports}{prometheus_networks}
    healthcheck:
  ...
  grafana:
    ...
    volumes:
      ...
      - {volume_prefix}_grafana_data:/var/lib/grafana{certs_volume}{grafana_ports}{grafana_networks}
    depends_on:
      grafana-init:
  ...
  alertmanager:
    ...
    command:
      ...
      - '--storage.path=/alertmanager'{alertmanager_web_config_cmd}
    volumes:
      - ./alertmanager/alertmanager.yml:/etc/alertmanager/alertmanager.yml:ro{alertmanager_web_config_vol}
      - {volume_prefix}_alertmanager_data:/alertmanager{certs_volume}
    ports:
      - "9093:9093"
    networks:
      - {network_name}
...
"#,
        ...
        // TLS-related config (skipped in development mode)
        certs_volume = certs_volume,
        prometheus_web_config_cmd = prometheus_web_config_cmd,
        prometheus_web_config_vol = prometheus_web_config_vol,
        alertmanager_web_config_cmd = alertmanager_web_config_cmd,
        alertmanager_web_config_vol = alertmanager_web_config_vol,
