...
impl AlertRules {
    /// Create default alert rules for a FedRAMP profile
...
fn generate_prometheus_rules(config: &AlertRules, fedramp: &FedRampConfig, app_name: &str) -> String {
...
fn generate_alertmanager_config(fedramp: &FedRampConfig, app_name: &str) -> String {
    format!(
...
mod tests {
    ...
    fn test_default_config_moderate() {
        let config = AlertRules::default_for_profile(&FedRampProfile::Moderate);
